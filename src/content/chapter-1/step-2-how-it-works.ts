...
export default {
    blocks: [
...

        {
            ...
            audience: "explorer",
            content:
                "The whole call runs on three main pieces working together -- the phone network on one side, the AI on the other, and a server in the middle passing words between them. Here is what each one is doing.",
        },
...

        {
            ...
            audience: "builder",
            content:
                "**Conversation Relay (STT + TTS)** — The bridge between voice and text. It uses **[Deepgram](https://deepgram.com/)** for speech-to-text (transcribing what the caller says) and **[ElevenLabs](https://elevenlabs.io/)** for text-to-speech (turning your text into natural-sounding audio) by default. It also detects interruptions mid-utterance. You can swap providers later — other options include Google (STT and TTS) and Amazon (TTS only).",
        },

        {
            ...
            variant: "info",
            content:
                "Twilio also offers **[Media Streams](https://www.twilio.com/docs/voice/media-streams)**, which give you raw audio bytes instead of transcribed text. That's the right choice if you want to run your own STT/TTS pipeline — or plug in a speech-to-speech (STS) model like [OpenAI Realtime](https://platform.openai.com/docs/guides/realtime) that takes audio in and returns audio out, bypassing the text middle-step entirely. The tradeoff: you manage audio codecs, buffering, voice activity detection, and barge-in yourself. Conversation Relay abstracts all of that — your server only deals with text over a WebSocket.",
        },
...

        {
            ...
            audience: "builder",
            content:
                "**LLM** — The language model powering your agent's intelligence. Your server sends it conversation history and a system prompt; it responds with streamed tokens to minimize latency. This workshop uses OpenAI's `gpt-5.4-nano`, but Twilio has no opinion about your LLM — Conversation Relay just hands your server transcribed text and expects streamed text tokens back. You can use Anthropic Claude, Google Gemini, a self-hosted Llama, a rules engine, whatever fits your agent. The only code that changes is how you call the model and iterate its response; the WebSocket handling stays the same.",
        },
