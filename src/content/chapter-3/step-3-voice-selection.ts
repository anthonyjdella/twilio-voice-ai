...
<Response>
  ...
  res.end(twiml);
...

    {
      type: "deep-dive",
      audience: "builder",
      title: "Models behind each provider (per Conversation Relay docs)",
      content:
        "`<ConversationRelay>` lets you pick a `ttsProvider`, a `voice`, and (for STT) a `speechModel`. Models themselves are selected either by Twilio's default, by the voice ID you pass, or -- for ElevenLabs -- by a hyphen suffix on the voice ID.\n\n**ElevenLabs TTS models.** Per the Conversation Relay voice configuration docs, voices from ElevenLabs use **`flash_v2_5`** (Flash 2.5) by default. Other supported models are `flash_v2`, `turbo_v2`, and `turbo_v2_5`. You can pick a non-default model by appending it to the voice ID with a hyphen, for example `voice=\"<voiceId>-turbo_v2_5\"`. Not every model works with every language; check ElevenLabs' site for model language support before switching.\n\n**Google TTS models.** Twilio's TTS voices page lists **Standard**, **WaveNet**, **Neural2**, and **Chirp3-HD** voice tiers for Google. Conversation Relay does not expose a separate model attribute -- you pick the tier implicitly by choosing a voice ID from that tier (for example `en-US-Wavenet-D` or `en-US-Chirp3-HD-Aoede`).\n\n**Amazon Polly TTS models.** Twilio's TTS voices page lists **Standard**, **Neural**, and **Generative** voice tiers for Polly. Same pattern as Google: choose a voice ID from the tier you want.\n\n**STT models (`speechModel` attribute).** Conversation Relay documents these defaults directly: `telephony` for Google, `nova-3-general` for Deepgram languages that support it, `nova-2-general` for other Deepgram languages. If you enable automatic multi-language detection (`multi`), Twilio requires `transcriptionProvider=\"Deepgram\"` with `speechModel` set to `nova-2-general` or `nova-3-general`.",
    },
